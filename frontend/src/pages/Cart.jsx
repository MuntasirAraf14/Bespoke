...
const Cart = () => {
  ...
  const { products, currency, cartItems, updateQuantity, delivery_fee } = useContext(ShopContext);
  
  const [cartData, setCartData] = useState([]);

  // --- Helper function to find product details by ID ---
  const getProductDetails = (id) => products.find((product) => product._id === id);
  ...
  }, [cartItems]); // Dependency array only needs cartItems now (products is static)
  ...
  if (cartData.length === 0) {
    return (
        <div className='py-20 text-center'>
            ...
            <Link to='/collection' className='mt-4 inline-block bg-black text-white px-6 py-3 rounded-lg hover:bg-gray-800 transition-colors'>
                Start Shopping
            </Link>
        </div>
    );
    ...
    <div className='border-t pt-14'>
      ...
      <div className='flex flex-col lg:flex-row gap-10'>
              ...
              return (
                <div key={`${item.id}-${item.size}`} 
                  ...
                  <div className='flex items-start gap-6'> 
                    <img src={productData.image[0]} alt={productData.name} className='w-20 h-20 object-cover border' />
                    <div>
                      <p className='font-medium'>{productData.name}</p>
                      <div className='flex items-center gap-5 mt-1 text-sm'>
                        <p className='font-semibold'>{currency}{productData.price}</p>
                        <p className='text-xs px-2 py-0.5 border bg-gray-100 rounded-sm'>Size: {item.size}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className='flex items-center justify-between'>
                    ...
                    {/* Item Total Price */}
                    <p className='hidden sm:block font-semibold'>{currency}{productData.price * item.quantity}</p>
        ...
        <div className='lg:w-2/5 p-6 border bg-gray-50 rounded-lg h-max'>
            <h2 className='text-xl font-semibold mb-6 border-b pb-3'>Cart Summary</h2>
            ...
            <div>
            <Link to="/place-order" className='mt-8 block w-full text-center bg-black text-white px-6 py-3 rounded-lg font-semibold hover:bg-gray-800 transition-colors'>
                PROCEED TO CHECKOUT
            </Link>
            </div>
        </div>
